  ...
  class Crawler
    ...
    def fetch(website)
      ...
      begin
        ...
        doc.css("div#link-report").to_a.each do |link|
          results[:summary] =  link.content.chomp if link.content
        end
        doc.css("strong.ll.rating_num").each do |num|
          results[:rate] = num.content if num
        end
        ...
        doc.css('div#info').each do |link|
          ...
          directors = content[/(?<=导演:)(.*)(?=上映日期)/] unless directors
        ...
        print "Pid:#{Process.pid}, fetch:#{website}, #{results[:name]} by #{results[:directors]},#{results[:rate]}\n"
...
puts "还有 #{Movie.where(:rate.gt => 8).count} 个没信息"
1.upto(30).each do |time|
  sites = []
