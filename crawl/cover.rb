...
def get_links(age, page, spec="tag")
  puts "grabing #{age} page #{page} ing"
  start = page.to_i * 20
  url = "http://movie.douban.com/#{spec}/#{age}?start=#{start}"
  begin
  ...
  links = []
  count = 0
  doc.css(".hd a").each do |link|
    href = link.attributes["href"].content
    if href =~ /subject/
      unless check_exist(href)
        links << href
      end
    end
  end
...
def check_exist(href)
  id = href.split("/")[-1]
  unless Top.where(movie_id: id).exists?
    puts "#{id} 不存在"
    return false
  else
    return true
